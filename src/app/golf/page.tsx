...
const CURRENT_CATEGORY_ID = "golf";
...
export async function generateMetadata(
  ...
  const newPageTitle = `Sportsurge ${categoryName} - Live ${categoryName} Tournaments, Leaderboards & Results`;
  const metaDescription = categoryDetails.metaDescription || `Get live ${categoryName.toLowerCase()} scores (PGA, LPGA, LIV), latest news, schedules, and in-depth updates on sportsurge. Your premier source for all ${categoryName.toLowerCase()} action.`;
  const canonicalUrlPath = `/${categoryId}`;
  const ogImageUrl = `https://placehold.co/1200x630.png`; 
  const baseKeywords = ['live scores', 'results', 'news', 'schedule', 'updates', 'sportsurge', 'tournaments', 'leaderboards', 'PGA', 'LPGA', 'LIV Golf'];
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `Live ${categoryName} tournaments, leaderboards, and results on Sportsurge`,
        },
      ],
      locale: 'en_US',
      type: 'website',
