...
const CURRENT_CATEGORY_ID = "boxing";
...
export async function generateMetadata(
  ...
  const metaDescription = categoryDetails.metaDescription || `Get live ${categoryName.toLowerCase()} results, latest news, schedules, and in-depth updates on sportsurge. Your premier source for all ${categoryName.toLowerCase()} action.`;
  const canonicalUrlPath = `/${categoryId}`;
  const ogImageUrl = `https://placehold.co/1200x630.png`; 
  const baseKeywords = ['live scores', 'results', 'news', 'schedule', 'updates', 'sportsurge', 'fights', 'fixtures', 'fight night'];
