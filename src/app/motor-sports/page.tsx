...
const CURRENT_CATEGORY_ID = "motor-sports";
...
export async function generateMetadata(
  ...
  const newPageTitle = `Sportsurge ${categoryName} - Live ${categoryName} Races, Fixtures & Results`;
  const metaDescription = categoryDetails.metaDescription || `Get live ${categoryName.toLowerCase()} (F1, IndyCar) results, latest news, schedules, and in-depth updates on sportsurge. Your premier source for all ${categoryName.toLowerCase()} action.`;
  const canonicalUrlPath = `/${categoryId}`;
  const ogImageUrl = `https://placehold.co/1200x630.png`; 
  const baseKeywords = ['live scores', 'results', 'news', 'schedule', 'updates', 'sportsurge', 'races', 'fixtures', 'F1', 'IndyCar', 'Formula 1'];
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `Live ${categoryName} races, fixtures, and results on Sportsurge`,
        },
      ],
      locale: 'en_US',
      type: 'website',
...
export default async function MotorSportsPage() {
  const categoryId = CURRENT_CATEGORY_ID;
  const categoryDetails = getCategoryDetails(categoryId, allSportCategoriesDetails);
