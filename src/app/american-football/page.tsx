...
const CURRENT_CATEGORY_ID = "american-football";
...
export async function generateMetadata(
  ...
  const categoryName = categoryDetails.name; // Should be "NFL"
  const newPageTitle = `Sportsurge ${categoryName} - Live ${categoryName} & College Football Matches, Fixtures & Results`;
  const metaDescription = categoryDetails.metaDescription || `Get live ${categoryName.toLowerCase()} & college football scores, latest news, schedules, and in-depth updates on sportsurge. Your premier source for all American football action.`;
  const canonicalUrlPath = `/${categoryId}`;
  const ogImageUrl = `https://placehold.co/1200x630.png`; 
  const baseKeywords = ['live scores', 'results', 'news', 'schedule', 'updates', 'sportsurge', 'matches', 'fixtures', 'college football', 'NCAAF'];
  const categoryKeywords = categoryName.toLowerCase().split(' ').map(k => `${categoryName} ${k}`); // NFL NFL
