...
const CURRENT_CATEGORY_ID = "wrestling-wwe";
...
export async function generateMetadata(
  ...
  const categoryName = categoryDetails.name; // Should be "WWE"
  const newPageTitle = `Sportsurge ${categoryName} - Live ${categoryName} Events, News & Results`;
  const metaDescription = categoryDetails.metaDescription || `Get live ${categoryName.toLowerCase()} updates, latest news, schedules, and results on sportsurge. Your premier source for all ${categoryName.toLowerCase()} action.`;
  const canonicalUrlPath = `/${categoryId}`;
  const ogImageUrl = `https://placehold.co/1200x630.png`; 
  const baseKeywords = ['live updates', 'results', 'news', 'schedule', 'sportsurge', 'events', 'Raw', 'Smackdown', 'NXT', 'PLE'];
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `Live ${categoryName} events, news, and results on Sportsurge`,
        },
      ],
      locale: 'en_US',
      type: 'website',
...
export default async function WrestlingWwePage() {
  const categoryId = CURRENT_CATEGORY_ID;
  const categoryDetails = getCategoryDetails(categoryId, allSportCategoriesDetails);
